...
- `ui.toolDisplay` is `"full"` (default) or `"compact"`. Full retains the outer Fabric TypeScript transcript; compact elevates the declared display name and description, hides that outer source even when expanded, and keeps bounded nested tool detail visible. Invalid values fall back to `"full"`. Change it under `/fabric settings` → **UI** or with `/fabric display full|compact [--global|--project]`; successful changes apply immediately to live and completed cards.
